...
namespace JastUsaLibrary.DownloadManager.Application
{
    ...
    {
        private readonly DownloadsPersistence _downloadsPersistence;
        public event EventHandler<Domain.Events.GameInstallationAppliedEventArgs> GameInstallationApplied;

        private void OnGameInstallationApplied(Game game, GameCache cache)
        {
            GameInstallationApplied?.Invoke(this, new Domain.Events.GameInstallationAppliedEventArgs(game, cache));
        }
        ...
        {
            _eventBus.Subscribe<DownloadItemStatusChangedEventArgs>(OnDownloadItemStatusChanged);
            _eventBus.Subscribe<DownloadItemProgressChangedEventArgs>(OnDownloadItemProgressChanged);
            _downloadsList.CollectionChanged += DownloadsList_CollectionChanged;
        }

        private void UnsubscribeToEvents()
        {
            _eventBus.Unsubscribe<DownloadItemStatusChangedEventArgs>(OnDownloadItemStatusChanged);
            _eventBus.Unsubscribe<DownloadItemProgressChangedEventArgs>(OnDownloadItemProgressChanged);
            _downloadsList.CollectionChanged -= DownloadsList_CollectionChanged;
        }

        private void OnDownloadItemStatusChanged(DownloadItemStatusChangedEventArgs args)
        {
            DownloadItemDownloadStatusChanged(args);
        }
        ...
        private async void DownloadsList_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                ...
                _eventBus.Publish(new DownloadsListItemsAddedEventArgs(newItemsList));
            ...
            {
                var oldItemsList = e.NewItems.Cast<DownloadItem>();
                _eventBus.Publish(new DownloadsListItemsRemovedEventArgs(oldItemsList));
        ...
        {
            var args = new GlobalProgressChangedEventArgs(
        ...
        private async Task RestorePersistingDownloads()
        {
            ...
            foreach (var downloadData in persistedDownloads)
            {
                ...
                var downloadItem = new DownloadItem(_jastAccountClient, downloadData, this, _eventBus);
        ...
        public async Task<bool> AddAssetToDownloadAsync(JastAssetWrapper assetWrapper)
        {
            var assetParentGameWrapper = _libraryCacheService.LibraryGames
                .FirstOrDefault(x => x.Assets.Contains(assetWrapper));
            ...
            var downloadItem = CreateNewDownloadItem(assetParentGameWrapper, assetWrapper);
        ...
        public DownloadItem CreateNewDownloadItem(JastGameWrapper gameWrapper, JastAssetWrapper jastAsset, CancellationToken cancellationToken = default)
        {
            ...
            return new DownloadItem(_jastAccountClient, downloadData, this, _eventBus);
        }
        ...
        private Uri GetAssetUri(GameLink downloadAsset, CancellationToken cancellationToken = default)
        {
            ...
            //{
            //    uri = _jastAccountClient.GetAssetDownloadLinkAsync(downloadAsset, a.CancelToken);
        ...
        public async Task MoveDownloadItemOnePlaceBeforeAsync(DownloadItem item)
        {
            ...
            try
            {
                ...
                if (newIndex >= 0)
                {
                    ...
                    _eventBus.Publish(new DownloadItemMovedEventArgs(item, currentIndex, newIndex));
                }
            }
            finally
            {
        ...
        private async void DownloadItemDownloadStatusChanged(Domain.Entities.DownloadItemStatusChangedEventArgs e)
        {
            var downloadStatus = e.NewStatus;
            var downloadItem = e.DownloadItem;
