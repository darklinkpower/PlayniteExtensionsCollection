...
namespace JastUsaLibrary.DownloadManager.Presentation
{
    public class DownloadsManagerViewModel : INotifyPropertyChanged, IDisposable
    {
        ...
        {
            _eventBus.Subscribe<DownloadItemMovedEventArgs>(OnDownloadItemMoved);
            _eventBus.Subscribe<DownloadItemStatusChangedEventArgs>(OnDownloadStatusChanged);
            _eventBus.Subscribe<DownloadsListItemsAddedEventArgs>(OnDownloadsListItemsAdded);
            _eventBus.Subscribe<DownloadsListItemsRemovedEventArgs>(OnDownloadsListItemsRemoved);
        }

        private void UnsubscribeToEvents()
        {
            _eventBus.Unsubscribe<DownloadItemMovedEventArgs>(OnDownloadItemMoved);
            _eventBus.Unsubscribe<DownloadItemStatusChangedEventArgs>(OnDownloadStatusChanged);
            _eventBus.Unsubscribe<DownloadsListItemsAddedEventArgs>(OnDownloadsListItemsAdded);
            _eventBus.Unsubscribe<DownloadsListItemsRemovedEventArgs>(OnDownloadsListItemsRemoved);
        }

        private void OnDownloadStatusChanged(Domain.Entities.DownloadItemStatusChangedEventArgs args)
        {
            NotifyPropertyChangedCommands();
        }

        private void OnDownloadItemMoved(DownloadItemMovedEventArgs args)
        {
            NotifyPropertyChangedCommands();
        }
        ...
        private void OnDownloadsListItemsAdded(DownloadsListItemsAddedEventArgs args)
        {
            NotifyPropertyChangedCommands();
        }
        ...
        public RelayCommand AddSelectedAssetToQueueCommand
        {
            get => new RelayCommand(async () =>
            {
                ...
                try
                {
                    var assetAddedToDownloads = await _downloadService.AddAssetToDownloadAsync(_selectedGameAssetWrapper);
                    if (assetAddedToDownloads)
                    {
