    ...
    public class DownloadData : INotifyPropertyChanged
    {
        ...
        public void SetUrl(Uri uri)
        {
            if (Url is null || FileName == Path.GetFileName(uri.LocalPath))
            {
                ...
                var unixExpiresTimeStamp = ExtractUnixTimestamp(uri.Query);
                if (!unixExpiresTimeStamp.HasValue)
                {
                    throw new InvalidOperationException($"Failed to extract the Unix timestamp from the URL: {uri.OriginalString}");
                }

                UrlExpiresTimeStamp = unixExpiresTimeStamp.Value;
            }
        }

        private static long? ExtractUnixTimestamp(string url)
        {
            var parts = url.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
            var lastPart = parts.Length > 1 ? parts[1] : null;

            if (long.TryParse(lastPart, out var timestamp))
            {
                return timestamp;
            }
